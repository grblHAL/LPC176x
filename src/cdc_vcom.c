...
static ErrorCode_t VCOM_bulk_out_hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
    ...
    case USB_EVT_OUT:
        ...
        usbBufferInput(pVcom->rx_buff, pVcom->rx_count);
...
static ErrorCode_t VCOM_SetLineState(USBD_HANDLE_T hCDC , uint16_t state)
{
    /* Called when line state is changed/set. Using it to know host connection state (DTR asserted) */
    usb_linestate.pin.dtr = (uint8_t)state;
...
ErrorCode_t vcom_init(USBD_HANDLE_T hUsb, USB_CORE_DESCS_T *pDesc, USBD_API_INIT_PARAM_T *pUsbParam)
{
    ...
    cdc_param.SetCtrlLineState = VCOM_SetLineState;
